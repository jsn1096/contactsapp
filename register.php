<?php
  ...
  if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (empty($_POST["name"]) || empty($_POST["email"]) || empty($_POST["password"])){
      ...
      // El bindparam("valor que toma la variable que se pone después de la coma" , " la variable que se le asigna al primer valor del paréntesis)
      // Cuando se pone execute el :email toma el valor de la variable $_POST["email"]
      ...
      if ($statement->rowCount() > 0) {
        ...
        ->execute([
          ":name" => $_POST["name"],
          ":email" => $_POST["email"],
          // Se pone eso para hashear la contraseña
